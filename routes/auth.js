...
router.get('/plex/poll', async (req, res) => {
  ...
  try {
    ...
    const allowed = await isAllowedOnServer(plexUser.id, data.authToken);
    if (!allowed) {
    ...
    res.json({ status: 'ok', user: req.session.user.username });
  } catch (err) {
...
router.get('/me', (req, res) => {
  ...
  res.json({ username: req.session.user.username, thumb: req.session.user.thumb });
});
