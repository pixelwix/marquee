...
// lib/mediaStorage.js is deliberately gitignored (real NAS mount details
// stay out of the public repo — see TODO.md/memory) so it doesn't exist on
// a fresh CI clone. Fall back to the same "no NAS volumes" shape
// mediaStorage.getVolumes() itself already returns when MEDIA_MOUNT_DIR
// isn't set, rather than letting the whole module fail to load.
let mediaStorage;
try {
  mediaStorage = require('./mediaStorage');
} catch (e) {
  mediaStorage = { getVolumes: async () => [] };
}
