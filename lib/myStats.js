...
// so they group by their own rating_key. `ratingKey` (the show key for
// episodes, the movie key otherwise) rides along so the Most Watched list
// can open the info modal / Played By on tap — no poster/synopsis is
// resolved here, that's a get_metadata round-trip made only on click.
function computeTopWatched(historyRows, limit = 3) {
  const groups = new Map();
  for (const r of historyRows) {
    const key = r.grandparent_rating_key || r.rating_key;
    if (!groups.has(key)) {
      groups.set(key, { title: r.grandparent_title || r.title, plays: 0, ratingKey: key ?? null });
    }
