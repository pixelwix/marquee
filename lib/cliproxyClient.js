...
// Tried first, before the same-account Claude call below — a Gemini Flash model via
// Antigravity, reached through CLIProxyAPI's OpenAI-compat passthrough (same host as
// the Claude path, just /chat/completions instead of /v1/messages). Antigravity shares
// the same personal OAuth quota as Claude/Codex (see memory cliproxyapi_providers), so
// this isn't "free" the way the old local Ollama tier was — it's here for latency/cost
// tiering, not to avoid metering. This task (suggest a fix given an alert's own text)
// needs no live tool access or WebSearch, so a Flash-tier model is a reasonable primary
// here. Returns null (never throws) on any failure, so complete() falls through to Claude.
async function completeWithAntigravity(prompt, maxTokens) {
  if (!process.env.CLIPROXY_URL || !process.env.CLIPROXY_API_KEY) return null;
  try {
    const res = await fetch(`${process.env.CLIPROXY_URL}/v1/chat/completions`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        model: process.env.ANTIGRAVITY_MODEL || 'gemini-3.1-flash-lite',
    ...
    console.error(`cliproxyClient: Antigravity attempt failed (falling back to Claude): ${err.message}`);
    return null;
  }
}
...
async function complete(prompt, { maxTokens = 500 } = {}) {
  const antigravityText = await completeWithAntigravity(prompt, maxTokens);
