...
// recipient throttled send (see lib/mailer.js) spans a real boundary.
// `now` defaults to the actual moment this recipient's email is rendered
// (called live during routes/recap.js's per-user send loop, not once for
// the whole batch), same testable-with-a-fixed-clock shape as
// arr-health-watchdog.mjs's restartBlipFor. Uses the container's own local
// time (already America/Chicago here, confirmed live via `date` inside the
// container), not UTC — unlike the month label above, which is intentionally
// UTC-pinned since it's a calendar boundary, not a time-of-day one.
