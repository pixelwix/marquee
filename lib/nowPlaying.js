...
const streamOrigins = require('./streamOrigins');
...
async function refresh() {
  ...
  refreshInFlight = (async () => {
    try {
      ...
      // A session key not already in the previous cache is a stream that
      // just started — record its origin exactly once here, before
      // mapSession() (which deliberately never sees ip_address_public at
      // all, see the comment there) gets a chance to run.
      for (const s of sessions) {
        if (!cache.has(s.session_key)) {
          streamOrigins.record(s.session_key, s.ip_address_public);
        }
      }
