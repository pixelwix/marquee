...
const { sanitizeSession, getPrivacyConfigFromEnv } = require('./privacy');

// Per-client context registered via addClient() is the viewer's own
// req.session.user — sanitized here (not in routes/tautulli.js) so the live
// SSE 'full' broadcast gets the exact same per-viewer privacy treatment as
// the initial REST snapshot, not just the page-load fetch. `snap` is computed
// once per broadcast/connect and reused across every client's own transform.
function sanitizeSnapshot(snap, context) {
  const config = getPrivacyConfigFromEnv();
  return { ...snap, sessions: snap.sessions.map((s) => sanitizeSession(s, context, config)) };
}
...
function mapSession(s) {
  return {
    ...
    // Exact playback position in ms — progress_percent above is a rounded
    // whole percent (Tautulli's own field), which is too coarse for a
    // second-accurate elapsed/ETA counter on the frontend (up to ~half a
    // percent of runtime off, several seconds on a typical episode).
    viewOffsetMs: Number(s.view_offset) || 0,
...
// Tautulli isn't required for sign-in the way Plex is, so it's realistic for a
// fresh deployment to reach here with it unset — skip cleanly rather than
// hitting a guaranteed-to-fail request every 10s. A single no-op refresh()
// call is cheap enough that gating just this one spot (rather than also
// guarding the setInterval in start()) is sufficient.
function tautulliConfigured() {
  return !!(process.env.TAUTULLI_URL && process.env.TAUTULLI_API_KEY);
}

// Plex itself is never optional in normal operation — the sign-in flow
// requires it — but a fresh/incomplete .env (first boot, before setup) can
// still reach this module with it unset. Guarded the same way as Tautulli
// above rather than assumed, so that case degrades to "Now Playing stays
// empty" instead of connectPlexSocket() throwing on a missing URL and taking
...
async function refresh() {
  ...
  refreshInFlight = (async () => {
    try {
      ...
      sse.broadcast('full', snapshot(), sanitizeSnapshot);
    } catch (err) {
...
function handlePlexNotification(sessionKey, state, viewOffset) {
  ...
  if (known && known.durationMs && state !== 'stopped') {
    ...
    known.viewOffsetMs = viewOffset;
    sse.broadcast('update', { sessionKey, state: known.state, progress: known.progress, viewOffsetMs: known.viewOffsetMs });
    return;
  }
...
function start() {
  if (!tautulliConfigured()) {
    console.log('now-playing: Tautulli not configured — Now Playing/Recently Watched will stay empty');
  }
  refresh();
  if (plexConfigured()) {
    connectPlexSocket();
  } else {
    console.log('now-playing: Plex not configured — Now Playing will stay empty and won\'t receive live updates');
  }
...
function wouldAcceptClient() {
  return sse.hasCapacity();
}

function addClient(res, context) {
  if (!sse.addClient(res, context)) return false;
  res.write(`event: full\ndata: ${JSON.stringify(sanitizeSnapshot(snapshot(), context))}\n\n`);
  return true;
}
...
module.exports = { start, addClient, removeClient, wouldAcceptClient, getSnapshot: snapshot };
