...
let cache = [];
let durationBySessionKey = new Map();
...
async function refresh() {
  ...
  refreshInFlight = (async () => {
    try {
      ...
      durationBySessionKey = new Map(sessions.map(s => [s.session_key, Number(s.duration) || 0]));
      cache = sessions.map(mapSession);
      broadcast('full', cache);
    } catch (err) {
...
function handlePlexNotification(sessionKey, state, viewOffset) {
  const known = cache.find(s => s.sessionKey === sessionKey);
  const duration = durationBySessionKey.get(sessionKey);
  if (known && duration && state !== 'stopped') {
    // Exact, not approximate — duration doesn't change mid-session, so this needs
    // no round trip to Tautulli.
    known.state = state;
    known.progress = Math.round((viewOffset / duration) * 100);
    broadcast('update', { sessionKey, state: known.state, progress: known.progress });
    return;
  }
...
function start() {
  refresh();
  connectPlexSocket();
  setInterval(refresh, 60000); // safety net in case a Plex event was ever missed
}
...
module.exports = { start, addClient, removeClient, getSnapshot: () => cache };
