...
// Called once at server startup (see server.js), before any real traffic
// arrives. Anything still 'sending' at that point can only be left over from
// a previous process — this one hasn't claimed anything yet — most likely a
// redeploy/restart landing mid-batch (routes/recap.js's background send job
// has no other way to notice its own process died). Without this, a stuck
// claim just sits silently until STALE_CLAIM_MS (15 min) lets some later,
// unrelated attempt for that exact user+period happen to reclaim it — which
// in practice may never happen, since a recap period is only ever normally
// sent to once. Marking it 'failed' here instead makes the interruption
// immediately visible in Recent Send History, with a clear reason, right
// when it happens rather than never.
async function reconcileAbandoned(now = Date.now()) {
  const conn = await withDb();
  const stuck = await all(conn, `SELECT period_key AS periodKey, user_id AS userId FROM recap_send_state WHERE status='sending'`);
  if (!stuck.length) return 0;
  await run(conn, `UPDATE recap_send_attempts SET status='failed', error='interrupted by server restart', completed_at=?
                   WHERE status='sending'`, [now]);
  await run(conn, `UPDATE recap_send_state SET status='failed' WHERE status='sending'`);
  return stuck.length;
}
...
module.exports = { STALE_CLAIM_MS, claim, finish, statusFor, history, reconcileAbandoned };
