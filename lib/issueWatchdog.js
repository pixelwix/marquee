...
async function check() {
  ...
  await checkSource('radarr:import', radarrClient.fetchImportQueue, r => ({
    ...
    wikiUrl: null,
    // Backs the alert's one-click "Force import" button (routes/alerts.js's
    // /actions/arr-force-import). Null when there's no downloadId to look up
    // manual-import candidates for — the button just isn't offered then.
    actionData: r.downloadId ? { type: 'arr-force-import', app: 'radarr', downloadId: r.downloadId } : null,
  }), scopes, newAlerts);

  await checkSource('sonarr:import', sonarrClient.fetchImportQueue, r => ({
    // episodeId, not r.id (a Sonarr queue-record id) — Sonarr can and does
    // recreate the queue record for the same still-stuck episode across
    // polls (e.g. a competing re-grab while the first stays stuck), which
    // churned this key and piled up duplicate "new" alerts for the identical
    // episode instead of updating one ongoing alert in place. episodeId is
    // stable for as long as the underlying problem actually is. Falls back
    // to r.id only for the same no-episode edge case fetchImportQueue itself
    // falls back on.
    key: `sonarr:import:${r.episodeId ?? r.id}`,
    app: 'sonarr',
    source: 'import',
    severity: 'warning',
    title: r.title,
    detail: r.reason || null,
    wikiUrl: null,
    actionData: r.downloadId ? { type: 'arr-force-import', app: 'sonarr', downloadId: r.downloadId } : null,
  }), scopes, newAlerts);
