...
function getDb() {
  ...
  ready = new Promise((resolve, reject) => {
    db.run(`CREATE TABLE IF NOT EXISTS alerts (
      ...
      // Added 2026-08-21 for the qbit-remove-torrents auto-fix action. CREATE TABLE IF
      // NOT EXISTS above only covers a brand-new db file — an already-migrated install
      // needs this added by hand. SQLite has no "ADD COLUMN IF NOT EXISTS", so the
      // duplicate-column error on a db that already has it is expected and ignored
