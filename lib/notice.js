...
// scheduled run instead of the ~60s-scoped one.
...
// Kometa's own error reporting. Fixed by keeping a real collection block
// whose plex_search is guaranteed to never match anything real — Kometa's
// own minimum_items floor (default 1) then simply never (re)creates it. This
// also gives a harmless fallback: if this app's direct Plex delete on clear
// ever failed silently, Kometa's own sync_mode would remove the stale item
// and delete the now-empty collection on its next run anyway.
...
# cleared. No notice is currently posted — this plex_search is written to
# deliberately match nothing (see the comment above this constant in
# lib/notice.js for why "collections: {}" doesn't work here).
collections:
  "📌 Server Announcement":
    plex_search:
      all:
        title: zzz-marquee-no-active-announcement-zzz
    sync_mode: sync
`;
...
async function clearKometaAnnouncement() {
  ...
  // Deleted directly rather than left for the scoped Kometa trigger below to
  // handle — see deleteAnnouncementCollection's own comment for why the
  // trigger can't actually do this part.
