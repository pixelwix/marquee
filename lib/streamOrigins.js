...
async function record(referenceId, ipAddress, at = Date.now()) {
  if (!ipAddress || isPrivateIp(ipAddress)) return;
  const geo = await lookupGeo(ipAddress);
  if (!geo) return;
  await run(
    `INSERT INTO stream_origins (reference_id, city, region, country, lat, lon, at) VALUES (?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(reference_id) DO UPDATE SET
       city = excluded.city, region = excluded.region, country = excluded.country,
       lat = excluded.lat, lon = excluded.lon`,
    [String(referenceId), geo.city, geo.region, geo.country, geo.lat, geo.lon, at]
...
async function topLocations({ limit = 12, range = 'ytd' } = {}) {
  const rows = await all(
    `SELECT city, region, country, AVG(lat) AS lat, AVG(lon) AS lon, COUNT(*) AS count
     FROM stream_origins
     WHERE at >= ?
