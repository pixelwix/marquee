...
const SERVICES = [
  ...
  },
  {
    ...
    fields: [
      { key: 'KOMETA_CONFIG_DIR', label: 'Config Directory', description: "Host path bind-mounted read-write into this container (see docker-compose.yml) — Kometa's own config directory. When set, posting or clearing the notice board also writes/removes a small collection YAML there, so Kometa mirrors the notice onto the Plex Home screen on its next scheduled run." }
    ]
  },
  {
